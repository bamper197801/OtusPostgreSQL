...
ALTER SYSTEM
postgres=# \q
otus@alex-1:~$ sudo pg_ctlcluster 15 main restart
...
```
otus@alex-1:~$ sudo su postgres
postgres@alex-1:/home/otus$ pgbench -i postgres
dropping old tables...
...
postgres@alex-1:/home/otus$ pgbench -c10 -P 60 -T 600 -U postgres postgres
...
otus@alex-1:~$ sudo tail -n 100 /var/log/postgresql/postgresql-15-main.log | grep checkpoint
...
postgres=# \q
otus@alex-1:~$ sudo pg_ctlcluster 15 main restart
otus@alex-1:~$ sudo -u postgres psql
...
```
otus@alex-1:~$ sudo pg_createcluster 15 check -- --data-checksums
...
otus@alex-1:~$ pg_ctlcluster 15 check start
...
otus@alex-1:~$ sudo systemctl start postgresql@15-check
otus@alex-1:~$ pg_lsclusters
Ver Cluster Port Status Owner    Data directory               Log file
...
postgres=# \q
postgres@alex-1:/home/otus$ exit
exit
otus@alex-1:~$ sudo systemctl stop postgresql@15-check
```
### Измените пару байт в таблице. Включите кластер и сделайте выборку из таблицы. 
```
otus@alex-1:~$ sudo sed -i 's/@/@123@/' /var/lib/postgresql/15/check/base/5/16391
otus@alex-1:~$ sudo systemctl start postgresql@15-check
otus@alex-1:~$  sudo su postgres
postgres@alex-1:/home/otus$  psql -p 5433
could not change directory to "/home/otus": Permission denied
psql (15.6 (Ubuntu 15.6-1.pgdg22.04+1))
Type "help" for help.
